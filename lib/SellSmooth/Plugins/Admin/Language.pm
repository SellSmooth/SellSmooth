package SellSmooth::Plugins::Admin::Language;
...
my $file = File::Spec->catfile( $FindBin::Bin, '..', 'plugins.d', 'admin_language.yml' );
open my $rfh, '<', $file or die "$file $!";
my $plugin_hash = LoadFile($file);
close $rfh;

my $path = '/' . SellSmooth::Plugins::Admin->plugin_hash()->{path} . '/languages';

get $path. '/list' => sub {

    template 'admin/list',
      { objects => SellSmooth::Core::Loaddataservice::list( 'Language', {}, { page => 1 } ), },
      { layout => 'admin' };
};

get $path. '/edit/:number' => sub {
    template 'admin/edit_language',
      { object => SellSmooth::Core::Loaddataservice::findByNumber( 'Language', params->{number} ), },
      { layout => 'admin' };
};
