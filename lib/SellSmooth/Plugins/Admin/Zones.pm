...
my $file = File::Spec->catfile( $FindBin::Bin, '..', 'plugins.d', 'admin_zones.yml' );
...
get $path. '/list' => sub {
    ...
    template 'admin/list',
      { objects => SellSmooth::Core::Loaddataservice::list( 'EconomicZone', {}, { page => 1 } ), },
      { layout => 'admin' };
};

get $path. '/edit/:number' => sub {
    template 'admin/edit',
      { object => SellSmooth::Core::Loaddataservice::findByNumber( 'EconomicZone', params->{number} ), },
      { layout => 'admin' };
};
